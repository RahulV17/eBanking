...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<Object> handle(org.springframework.dao.DataIntegrityViolationException exception) {
		// Duplicate transaction (replayed payment id) — idempotency guard
		return ResponseEntity.status(409).body(new ErrorDto("Transaction already processed (duplicate payment reference)"));
	}
	...
	public ResponseEntity<Object> handle(org.springframework.dao.OptimisticLockingFailureException exception) {
		return ResponseEntity.status(409).body(new ErrorDto("Concurrent update detected — please retry the operation."));
	}
	...
	public ResponseEntity<Object> handle(Exception exception) {
		...
		return ResponseEntity.status(500).body(new ErrorDto("Internal server error: " + exception.getMessage()));
	}
}